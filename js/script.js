//헤더 메뉴 선택
const sites = document.querySelector('.header-wrapper');
let currentSite;

function siteSelectHandler(e) {
const site = e.target;

if (currentSite) {
    currentSite.classList.remove('site-clicked');
}

site.classList.add('site-clicked');
currentSite = site;
}

sites.addEventListener('click', siteSelectHandler);
...
buttonLeft.addEventListener('click', () =>{
    ...
    clearInterval(interval); //기존 동작되던 interval wㅔ거
...
const dragging = (e) => {
    cardWrap.scrollLeft = e.pageX;
}
cardWrap.addEventListener("mousemove", dragging);